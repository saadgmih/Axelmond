...
assert.equal(
  isAllowedPushEndpointUrl("https://fcm.googleapis.com/fcm/send/device-token"),
  true,
);
assert.equal(
  isAllowedPushEndpointUrl("https://updates.push.services.mozilla.com/wpush/v2/gAAAAA"),
  true,
);
assert.equal(
  isAllowedPushEndpointUrl("https://web.push.apple.com/abc123"),
  true,
);
assert.equal(
  isAllowedPushEndpointUrl("https://wns2-bl2p.notify.windows.com/w/?token=abc"),
  true,
);
