...
if (!isSecurityRuntimeDatabaseAvailable()) {
  console.log("Security runtime upload rate tests skipped: DATABASE_URL missing");
  process.exit(0);
}
...
try {
  ...
  const responses = [];
  for (let i = 0; i < 3; i += 1) {
    responses.push(await fetch(`${handle.baseUrl}${UPLOAD_PATH}`, {
      method: "POST",
  ...
  assert.ok(statuses.slice(0, 2).every((status) => status !== 429), `Expected first requests not rate-limited, got ${statuses.join(",")}`);
