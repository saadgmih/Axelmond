...
rulesTest("db-schema", () => {
  assert.equal(resolvePgSchema("postgresql://user:pass@host/db?sslmode=require&schema=unicode"), DEFAULT_PG_SCHEMA);

  assert.equal(resolvePgSchema("postgresql://user:pass@host/db?schema=public"), "public");

  assert.equal(resolvePgSchema("postgresql://user:pass@host/db"), DEFAULT_PG_SCHEMA);

  assert.equal(resolvePgSchema("postgresql://user:pass@host/db?schema=bad-schema"), DEFAULT_PG_SCHEMA);
