...
rulesTest("security-runtime-livekit", async () => {
  ...
  try {
    ...
    // T1. Sans auth → 401
    {
      const response = await postLiveKit(handle.baseUrl, TOKEN_PATH, tokenBody(fixture), undefined, {
        forgedCsrf: true,
      });
    ...
    {
      const response = await authedFetch(handle.baseUrl, unenrolledSession, "POST", TOKEN_PATH, tokenBody(fixture));
    ...
    {
      const response = await authedFetch(handle.baseUrl, ownerSession, "POST", TOKEN_PATH, tokenBody(fixture));
    ...
    {
      const response = await authedFetch(handle.baseUrl, foreignSession, "POST", TOKEN_PATH, tokenBody(fixture));
    ...
    // T7. Admin → 200
    {
      const response = await authedFetch(handle.baseUrl, adminSession, "POST", TOKEN_PATH, tokenBody(fixture));
    ...
    // M1. Sans auth → 401
    {
      const response = await postLiveKit(handle.baseUrl, MODERATION_PATH, moderationBody(fixture), undefined, {
        forgedCsrf: true,
      });
