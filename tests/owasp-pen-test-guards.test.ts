...
rulesTest("owasp-pen-test-guards", () => {
  ...
  const owaspChecks: Array<{ id: string; ok: boolean; category: string }> = [
    { id: "A01-broken-access-control-rbac", ok: /requireRbac/.test(apiSource) && /canAccessApiRoute/.test(apiSource), category: "A01" },
    { id: "A01-ownership-verify-course", ok: /verifyCourseAccess/.test(apiSource), category: "A01" },
    { id: "A02-crypto-auth-token", ok: /verifyAuthToken/.test(apiSource) && /hashRefreshToken/.test(bootstrap), category: "A02" },
    ...
    { id: "A04-insecure-design-rate-limits", ok: /globalRateLimiter/.test(bootstrap) && /authRateLimiter/.test(bootstrap), category: "A04" },
    ...
    { id: "A08-integrity-upload-validation", ok: /isDangerousFile/.test(uploadthing) && /isValidMimeType/.test(uploadthing), category: "A08" },
    { id: "A10-ssrf-live-allowlist", ok: /isAllowedLiveResourceHost/.test(liveValidation), category: "A10" },
    { id: "A10-ssrf-avatar-allowlist", ok: /isAllowedAvatarHost/.test(fs.readFileSync("src/avatar-security.ts", "utf8")), category: "A10" },
    { id: "csrf-double-submit", ok: /CSRF_TOKEN_INVALID/.test(authCsrf) && /headerToken !== cookieToken/.test(authCsrf.replace(/\s+/g, " ")), category: "CSRF" },
    ...
    { id: "rtl-testing-library", ok: packageJson.devDependencies?.["@testing-library/react"] !== undefined, category: "Testing" },
  ];
  ...
  assert.equal(
    failed.length,
    0,
    `OWASP guard failures: ${failed.map((entry) => entry.id).join(", ")}`,
  );
