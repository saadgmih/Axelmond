...
export function registerQuizRoutes(app: Express, ctx: RouteContext): void {
  ...
  app.post("/api/courses/:courseId/modules/:moduleId/quiz-attempts", requireAuth, requireRbac, async (req, res) => {
    ...
    api.logDb("INFO", "Quiz attempt recorded", { courseId: course.id, moduleId: module.id, userId: authUser.id, score, total, scoreOutOf20 });
  ...
  app.post("/api/quizzes/:quizId/attempts", requireAuth, validateBody(api.quizAttemptSchema), async (req, res) => {
    ...
    api.logDb("INFO", "Flexible quiz attempt recorded", { quizId: quiz.id, courseId: quiz.courseId, userId: authUser.id, score, total, scoreOutOf20 });
  ...
  app.get("/api/courses/:courseId/quizzes", requireAuth, async (req, res) => {
    ...
    api.logDb("INFO", "Quiz list fetched", { courseId, count: quizList.length, userId: authUser.id, role: authUser.role });
  ...
  app.post("/api/courses/:courseId/quizzes", requireAuth, requireRbac, validateBody(api.quizSchema), async (req, res) => {
    ...
    api.logDb("INFO", "Quiz created", { quizId: quiz.id, courseId, moduleId: quiz.moduleId, sectionId: quiz.sectionId, userId: authUser.id });
  ...
  app.patch("/api/quizzes/:quizId", requireAuth, requireRbac, validateBody(api.quizPatchSchema), async (req, res) => {
    ...
    if (quiz.published && !existingQuiz.published) {
      ...
      await api.notifyEnrolledStudentsForCourse(quiz.courseId, {
  ...
  app.post("/api/quizzes/:quizId/questions", requireAuth, requireRbac, validateBody(api.quizQuestionSchema), async (req, res) => {
