...
export function registerObjectivesRoutes(app: Express, ctx: RouteContext): void {
  ...
  app.post("/api/me/study-schedule", requireAuth, requireRbac, validateBody(api.studentStudyScheduleSchema), async (req, res) => {  
    const authUser = (req as any).authUser as AppUser;  
    if (authUser.role !== "STUDENT") {  
      res.status(403).json({ error: "Emploi du temps d'étude réservé aux étudiants" });  
      return;  
    ...
    await api.logAudit(authUser.id, authUser.email, "CREATE_STUDY_SCHEDULE_SESSION", "StudentStudyScheduleSession", created.id, { dayOfWeek: created.dayOfWeek }, req.ip);  
    res.status(201).json(api.serializeStudentStudySession(created));  
  });  
    
  // PUT /api/me/study-schedule/:id  
  app.put("/api/me/study-schedule/:id", requireAuth, requireRbac, validateBody(api.studentStudyScheduleSchema), async (req, res) => {  
    const authUser = (req as any).authUser as AppUser;  
    if (authUser.role !== "STUDENT") {  
      res.status(403).json({ error: "Emploi du temps d'étude réservé aux étudiants" });  
      return;  
    ...
    await api.logAudit(authUser.id, authUser.email, "UPDATE_STUDY_SCHEDULE_SESSION", "StudentStudyScheduleSession", updated.id, { dayOfWeek: updated.dayOfWeek }, req.ip);  
  ...
  app.delete("/api/me/study-schedule/:id", requireAuth, requireRbac, async (req, res) => {  
    ...
    await api.logAudit(authUser.id, authUser.email, "DELETE_STUDY_SCHEDULE_SESSION", "StudentStudyScheduleSession", owned.id, {}, req.ip);  
    res.json({ ok: true });  
  });  
  ...
  async function listStudentObjectives(studentId: string) {  
    ...
    return api.sortStudentObjectives(objectives as any).map((objective) => api.serializeStudentObjective(objective as any));  
  }  
  ...
  app.post("/api/me/objectives", requireAuth, requireRbac, validateBody(api.studentObjectiveSchema), async (req, res) => {  
    const authUser = (req as any).authUser as AppUser;  
    if (authUser.role !== "STUDENT") {  
      res.status(403).json({ error: "Objectifs réservés aux étudiants" });  
      return;  
    ...
    await api.logAudit(authUser.id, authUser.email, "CREATE_STUDENT_OBJECTIVE", "StudentObjective", created.id, { status: created.status }, req.ip);  
    res.status(201).json(api.serializeStudentObjective(created as any));  
  });  
    
  // PUT /api/me/objectives/:id  
  app.put("/api/me/objectives/:id", requireAuth, requireRbac, validateBody(api.studentObjectiveSchema), async (req, res) => {  
    const authUser = (req as any).authUser as AppUser;  
    if (authUser.role !== "STUDENT") {  
      res.status(403).json({ error: "Objectifs réservés aux étudiants" });  
      return;  
    ...
    await api.logAudit(authUser.id, authUser.email, "UPDATE_STUDENT_OBJECTIVE", "StudentObjective", updated.id, { status: updated.status }, req.ip);  
  ...
  app.patch("/api/me/objectives/:id/complete", requireAuth, requireRbac, async (req, res) => {  
    ...
    await api.logAudit(authUser.id, authUser.email, "COMPLETE_STUDENT_OBJECTIVE", "StudentObjective", updated.id, {}, req.ip);  
    res.json({  
  ...
  app.delete("/api/me/objectives/:id", requireAuth, requireRbac, async (req, res) => {  
    ...
    await api.logAudit(authUser.id, authUser.email, "DELETE_STUDENT_OBJECTIVE", "StudentObjective", owned.id, {}, req.ip);  
    res.json({ ok: true });  
  });  
