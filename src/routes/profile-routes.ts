...
export function registerProfileRoutes(app: Express, ctx: RouteContext): void {
  ...
  app.put("/api/me/profile", requireAuth, requireRbac, async (req, res) => {  
    ...
    if ("role" in req.body || "userId" in req.body) {  
      api.logSecurity("WARN", "Academic profile immutable fields ignored", { userId: authUser.id, fields: Object.keys(req.body).filter((field) => field === "role" || field === "userId") });  
    }  
  ...
  app.post("/api/me/schedule", requireAuth, requireRbac, validateBody(api.scheduleSessionSchema), async (req, res) => {  
    ...
    await api.logAudit(authUser.id, authUser.email, "CREATE_SCHEDULE_SESSION", "ProfessorScheduleSession", created.id, { dayOfWeek: created.dayOfWeek }, req.ip);  
    res.status(201).json(api.serializeScheduleSession(created));  
  });  
    
  // PUT /api/me/schedule/:id  
  app.put("/api/me/schedule/:id", requireAuth, requireRbac, validateBody(api.scheduleSessionSchema), async (req, res) => {  
    const authUser = (req as any).authUser as AppUser;  
    if (!api.canAccessAcademicProfile(authUser.role)) {  
      res.status(403).json({ error: "Emploi du temps réservé aux professeurs et chercheurs" });  
      return;  
    ...
    await api.logAudit(authUser.id, authUser.email, "UPDATE_SCHEDULE_SESSION", "ProfessorScheduleSession", updated.id, { dayOfWeek: updated.dayOfWeek }, req.ip);  
  ...
  app.delete("/api/me/schedule/:id", requireAuth, requireRbac, async (req, res) => {  
    ...
    await api.logAudit(authUser.id, authUser.email, "DELETE_SCHEDULE_SESSION", "ProfessorScheduleSession", owned.id, {}, req.ip);  
    res.json({ ok: true });  
  });  
  ...
  app.delete("/api/me/avatar", requireAuth, requireRbac, async (req, res) => {  
    ...
    res.json({ user: user ? api.toAppUser(user) : { ...authUser, avatarUrl: undefined }, message: "Photo de profil supprimée" });  
  });  
  ...
  app.post("/api/me/password", requireAuth, requireRbac, validateBody(api.passwordChangeSchema), async (req, res) => {  
    ...
    if (!validPassword) {  
      api.logSecurity("WARN", "Academic password update denied", { userId: authUser.id, reason: "invalid_current_password" });  
