...
export function registerLiveRoutes(app: Express, ctx: RouteContext): void {
  ...
  app.post("/api/livekit/token", requireAuth, async (req, res) => {
    ...
    if (!access.ok) {
      ...
      api.logLiveKit("WARN", "Token denied", { userId: authUser.id, courseId: Number(courseId), status: access.status });
  ...
  app.post("/api/livekit/attendance/leave", requireAuth, validateBody(api.liveAttendanceLeaveSchema), async (req, res) => {
  ...
  app.post("/api/livekit/events", requireAuth, validateBody(api.liveEventSchema), async (req, res) => {
    ...
    api.logLiveKit("INFO", "Live event stored", { roomName: session.roomName, userId: authUser.id, action: req.body.action });
  ...
  app.post("/api/livekit/moderation", requireAuth, requireRbac, validateBody(api.liveModerationSchema), async (req, res) => {
    ...
    try {
      ...
      api.logLiveKit("INFO", "Live moderation applied", { roomName: session.roomName, actorId: authUser.id, action: req.body.action, targetIdentity: req.body.targetIdentity });
  ...
  app.post("/api/livekit/sync", requireAuth, requireRbac, async (req, res) => {
    ...
    try {
      ...
      api.logLiveKit("INFO", "Live sync relayed", { roomName: session.roomName, userId: authUser.id, type: validated.type });
      res.json({ ok: true });
    } catch (err: any) {
