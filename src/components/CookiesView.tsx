...
const CookieTypeCard: React.FC<{
  icon: React.ReactNode; emoji: string; category: string; title: string;
  description: string; items: string[];
  border: string; bg: string; titleColor: string; chipColor: string;
  required?: boolean; delay: number; inView: boolean;
}> = ({ icon, emoji, category, title, description, items, border, bg, titleColor, chipColor, required, delay, inView }) => (
  <FadeIn inView={inView} delay={delay}>
    <div className={`h-full rounded-2xl border p-6 space-y-4 ${border} ${bg} hover:-translate-y-0.5 transition-transform duration-200`}>
      {/* Header */}
      <div className="flex items-start justify-between gap-3">
        ...
        {required !== undefined && (
          <span className={`text-[9px] font-black uppercase tracking-widest border px-2 py-0.5 rounded-full flex-shrink-0 ${chipColor}`}>
...
const SectionHeader: React.FC<{
  number: string; icon: React.ReactNode; title: string; accentClass: string;
}> = ({ number, icon, title, accentClass }) => (
  <div className="flex items-center gap-3 mb-6">
    <div className={`w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 ${accentClass}`}>
      {icon}
    </div>
...
export default function CookiesView() {
  ...
  const cookieTypes = [
    {
      ...
      category: "Cookies",
      title: "Essentiels",
      description: "Indispensables au fonctionnement sécurisé de la plateforme. Ils ne peuvent pas être désactivés sans compromettre votre accès aux services.",
      items: [
    ...
    },
    {
      ...
      category: "Cookies",
      title: "Fonctionnels",
      description: "Améliorent votre expérience en mémorisant vos choix personnels. Peuvent être désactivés, mais certaines fonctionnalités seront réduites.",
      items: [
    ...
    },
    {
      ...
      category: "Cookies",
      title: "Analytiques",
      description: "Collectent des données agrégées et anonymisées pour comprendre l'usage de la plateforme et améliorer les performances pédagogiques.",
      items: [
  ...
  const purposes = [
    { icon: <Fingerprint className="w-4 h-4 text-indigo-400" />, label: "Maintenir la connexion utilisateur", desc: "Votre session reste active entre les pages sans vous obliger à vous reconnecter à chaque navigation.", color: "border-indigo-800/30 bg-indigo-950/15" },
    { icon: <Lock className="w-4 h-4 text-emerald-400" />, label: "Sécuriser les accès", desc: "Vérification de l'identité côté serveur à chaque requête et protection contre les accès non autorisés.", color: "border-emerald-800/30 bg-emerald-950/15" },
    { icon: <Settings className="w-4 h-4 text-violet-400" />, label: "Mémoriser les préférences", desc: "Vos choix d'interface (thème, langue, paramètres) sont retenus pour les sessions suivantes.", color: "border-violet-800/30 bg-violet-950/15" },
    { icon: <BarChart2 className="w-4 h-4 text-sky-400" />, label: "Optimiser les performances", desc: "Réduction des temps de chargement et amélioration de la réactivité de la plateforme.", color: "border-sky-800/30 bg-sky-950/15" },
    { icon: <Eye className="w-4 h-4 text-pink-400" />, label: "Améliorer l'expérience utilisateur", desc: "Personnalisation de l'interface et adaptation du parcours pédagogique selon votre profil.", color: "border-pink-800/30 bg-pink-950/15" },
  ];
        ...
        >
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-8">
            ...
            <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 space-y-3 flex-shrink-0 min-w-0 sm:min-w-[210px]">
              ...
              <div>
                <div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold">Cookies tiers publicitaires</div>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-10 py-10 space-y-10">
          ...
          <FadeIn inView={s1.inView}>
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-7 md:p-9">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {[
                  { icon: <Globe className="w-5 h-5 text-indigo-400" />, title: "Bon fonctionnement", desc: "Les cookies permettent à la plateforme de fonctionner correctement — gestion des sessions, authentification sécurisée et navigation fluide entre les pages.", border: "border-indigo-800/30 bg-indigo-950/15" },
                  { icon: <Eye className="w-5 h-5 text-violet-400" />, title: "Expérience améliorée", desc: "Ils mémorisent vos préférences d'affichage, votre langue et vos paramètres personnels pour vous offrir une expérience académique cohérente.", border: "border-violet-800/30 bg-violet-950/15" },
                  { icon: <Shield className="w-5 h-5 text-emerald-400" />, title: "Sécurité renforcée", desc: "Certains cookies contribuent à la sécurité de votre compte en vérifiant votre identité à chaque requête et en protégeant contre les accès non autorisés.", border: "border-emerald-800/30 bg-emerald-950/15" },
        ...
        <div ref={s2.ref}>
          <FadeIn inView={s2.inView}>
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-7 md:p-9">
              <SectionHeader number="02" title="Qu'est-ce qu'un cookie ?"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-center">
                <div className="space-y-4">
                  <p className="text-[13px] text-slate-300 leading-relaxed">
                    ...
                    texte stocké sur votre appareil — ordinateur, tablette ou smartphone — lorsque vous visitez
                    un site web ou utilisez une application.
                  </p>
                  <p className="text-[13px] text-slate-300 leading-relaxed">
                    Ce fichier contient des informations que la plateforme peut lire lors de votre prochaine visite,
        ...
        <div ref={s3.ref}>
          <FadeIn inView={s3.inView}>
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-7 md:p-9">
              <SectionHeader number="03" title="Types de cookies utilisés"
        ...
        <div ref={s4.ref}>
          <FadeIn inView={s4.inView}>
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-7 md:p-9">
              <SectionHeader number="04" title="Finalités des cookies"
        ...
        <div ref={s5.ref}>
          <FadeIn inView={s5.inView}>
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-7 md:p-9">
              <SectionHeader number="05" title="Gestion des cookies"
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                {managementSteps.map(({ icon, title, desc, color }) => (
                  <div key={title} className={`rounded-2xl border p-5 space-y-3 ${color} hover:scale-[1.02] transition-transform duration-200`}>
              ...
              <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-5">
                <div className="text-[10px] uppercase tracking-widest text-slate-500 font-bold mb-3">Accès aux paramètres par navigateur</div>
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  {browserGuide.map(({ browser, path, icon }) => (
                    <div key={browser} className="bg-slate-900 border border-slate-800 rounded-xl p-3 space-y-1.5 text-center">
              ...
              <div className="mt-4 bg-amber-950/15 border border-amber-800/25 rounded-xl px-4 py-3 flex items-start gap-2.5">
                ...
                <p className="text-[11px] text-slate-400">
                  ...
                  <strong className="text-white"> essentiels</strong> empêchera votre connexion à la plateforme.
                  Seuls les cookies fonctionnels et analytiques peuvent être refusés sans impacter l'accès aux services de base.
                </p>
              </div>
            </div>
          </FadeIn>
          ...
          <FadeIn inView={s6.inView}>
            <div className="relative overflow-hidden bg-slate-900 border border-emerald-900/40 rounded-3xl p-7 md:p-9">
              ...
              <div className="relative">
                <SectionHeader number="06" title="Protection des données"
                ...
                <div className="bg-slate-950/60 border border-slate-800 rounded-2xl p-5">
                  <div className="text-[10px] uppercase tracking-widest text-emerald-500 font-bold mb-3">Mesures de sécurité complémentaires</div>
        ...
        <div ref={s7.ref}>
          <FadeIn inView={s7.inView}>
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-7 md:p-9">
              <SectionHeader number="07" title="Durée de conservation"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-5">
                <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-5 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="ml-auto text-[10px] font-black uppercase tracking-widest border px-2 py-0.5 rounded-full text-sky-300 border-sky-500/30 bg-sky-500/10">Temporaire</span>
                  </div>
                  <p className="text-[12px] text-slate-400 leading-relaxed">
                    Ces cookies expirent automatiquement à la fermeture du navigateur ou lors de la
                    déconnexion explicite. Ils ne laissent aucune trace après votre session.
                  </p>
                  <div className="text-[11px] text-slate-500">Exemple : jeton de session, état de navigation temporaire</div>
                </div>
                <div className="bg-slate-950/50 border border-slate-800 rounded-2xl p-5 space-y-3">
                  <div className="flex items-center gap-2">
                    <Database className="w-4 h-4 text-violet-400" />
                    <span className="text-white text-sm font-bold">Cookies persistants</span>
                    <span className="ml-auto text-[10px] font-black uppercase tracking-widest border px-2 py-0.5 rounded-full text-violet-300 border-violet-500/30 bg-violet-500/10">Persistant</span>
                  </div>
                  <p className="text-[12px] text-slate-400 leading-relaxed">
                    Certains cookies restent jusqu'à leur expiration programmée ou leur suppression
                    manuelle, afin de mémoriser vos préférences entre les sessions.
                  </p>
                ...
                <table className="w-full text-left">
                  <thead>
                    <tr className="bg-slate-800/60">
                      ...
                      <th className="px-4 py-3 text-[10px] uppercase tracking-widest text-slate-400 font-black hidden sm:table-cell">Type</th>
                      <th className="px-4 py-3 text-[10px] uppercase tracking-widest text-slate-400 font-black">Durée</th>
                      <th className="px-4 py-3 text-[10px] uppercase tracking-widest text-slate-400 font-black hidden md:table-cell">Catégorie</th>
                    </tr>
                  </thead>
                    ...
                    {retentionData.map(({ name, type, duration, category }) => (
                      <tr key={name} className="hover:bg-slate-800/20 transition-colors">
                        <td className="px-4 py-3">
                          <code className="text-[11px] text-amber-300 bg-amber-900/20 border border-amber-800/30 px-2 py-0.5 rounded-md font-mono">{name}</code>
                        </td>
                        ...
                        <td className="px-4 py-3 hidden md:table-cell">
                          <span className={`text-[10px] font-black uppercase tracking-widest border px-2 py-0.5 rounded-full ${
                            category === "Essentiel"  ? "text-emerald-300 border-emerald-500/30 bg-emerald-500/10" :
                            category === "Fonctionnel" ? "text-violet-300 border-violet-500/30 bg-violet-500/10" :
          ...
          <FadeIn inView={s8.inView}>
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-7 md:p-9">
              ...
              <p className="text-[13px] text-slate-400 leading-relaxed mb-6">
                Pour toute question relative à notre politique de cookies, à l'exercice de vos droits ou
                à la protection de vos données personnelles, plusieurs canaux sont à votre disposition.
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                <div className="bg-indigo-950/20 border border-indigo-800/30 rounded-2xl p-5 space-y-3">
                  ...
                  <p className="text-[12px] text-slate-400 leading-relaxed">
                    Consultez notre politique de confidentialité complète pour comprendre comment nous
