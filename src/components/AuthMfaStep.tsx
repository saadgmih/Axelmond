  ...
  return (
    <div className="mx-auto w-full max-w-md space-y-6 rounded-3xl border border-slate-800 bg-slate-950/80 p-6 shadow-2xl">
      ...
      {errorMsg && (
        <div
          role="alert"
          className="rounded-xl border border-emerald-500/30 bg-emerald-950/40 px-4 py-3 text-xs text-emerald-200"
        >
          {errorMsg}
        </div>
      )}
      ...
      <form onSubmit={handleTotpSubmit} className="space-y-4">
        <label className="block space-y-2">
          ...
          <input
            ...
            placeholder="123456"
            className="w-full rounded-xl border border-slate-700 bg-slate-900 px-4 py-3 text-center font-mono text-lg tracking-[0.3em] text-white outline-none focus:border-emerald-500"
          />
        </label>
        <button
          type="submit"
          disabled={isLoading || code.trim().length < 6}
          className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-3 text-sm font-bold text-white disabled:opacity-50"
        >
      ...
      {passkeysAvailable && (
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-700 bg-slate-900 px-4 py-3 text-sm font-bold text-slate-100 hover:border-emerald-500/50"
        >
