...
function ProductivityStatCard({
  ...
  return (
    <article className="rounded-2xl border border-white/[0.08] bg-[#0b241f]/80 p-4 shadow-xl shadow-black/20">
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="rounded-xl border border-teal-400/10 bg-teal-500/10 p-2 text-teal-200">{icon}</div>
      </div>
    </article>
  );
}
...
export function ObjectivesStatsSection({
  ...
  return (
    <>
      <section className="rounded-2xl border border-white/[0.08] bg-[#031512] p-4 sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <p className="text-[10px] font-black uppercase tracking-widest text-teal-300">Progression hebdomadaire</p>
      ...
      {(dueSoonObjectives.length > 0 || overdueObjectives.length > 0) && (
        <section className="grid grid-cols-1 gap-3 lg:grid-cols-2">
          ...
          {dueSoonObjectives.length > 0 && (
            <div className="rounded-2xl border border-lime-400/20 bg-lime-950/20 p-4">
              <h2 className="inline-flex items-center gap-2 text-sm font-black text-lime-200">
                <Clock3 className="h-4 w-4" />
                Proches de la date limite
              </h2>
              <div className="mt-3 space-y-2">
                {dueSoonObjectives.slice(0, 3).map((objective) => (
                  <p key={objective.id} className="text-xs font-semibold text-lime-100/90">
                    {objective.title} · {formatDateTime(objective.endAt)}
                  </p>
                ))}
              </div>
      ...
      <section className="rounded-2xl border border-teal-400/10 bg-[#0b241f]/70 p-4 sm:p-5">
        <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
          ...
          <Headphones className="hidden h-8 w-8 text-teal-300 sm:block" />
        </div>
      </section>
    </>
  );
