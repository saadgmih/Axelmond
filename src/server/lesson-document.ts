...
export async function streamLessonContentDocument(contentId: string, authUser: AppUser) {
  ...
  if (content.type !== "PDF") {
    return { ok: false as const, status: 400, error: "Ce contenu n'est pas un document PDF" };
  }
  ...
  const attachment = content.attachments.find((item) => item.type === "PDF") ?? content.attachments[0];
  if (!attachment) {
    return { ok: false as const, status: 404, error: "Document PDF introuvable" };
  }
  ...
  if (!upstream.ok) {
    return { ok: false as const, status: 502, error: "Impossible de récupérer le document PDF" };
  }

  const contentLength = Number(upstream.headers.get("content-length") || 0);
  if (contentLength > LESSON_DOCUMENT_MAX_BYTES) {
    return { ok: false as const, status: 413, error: "Document PDF trop volumineux pour l'aperçu intégré" };
  }
  ...
  return {
    ok: true as const,
    fileName: attachment.fileName || `${content.title}.pdf`,
    bytes,
  };
}
